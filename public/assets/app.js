(() => {
  ...
  function renderCatalog() {
    ...
    const renderCard = skill => {
      ...
      return `<article class="skill-card"><a class="card-open" href="${escapeHTML(skill.sourceUrl)}" target="_blank" rel="noopener noreferrer" data-skill="${escapeHTML(skill.id)}" title="点击后进入技能库，按照 README 使用说明开始操作。" aria-label="打开 ${escapeHTML(skill.sourceLabel)} GitHub 技能库"><div class="card-top"><span class="skill-icon ${escapeHTML(skill.categoryKey)}">${icon(skill.icon)}</span><span class="card-index">REPO / ${String(catalog.indexOf(skill) + 1).padStart(3, '0')}</span></div><span class="card-hover-hint" aria-hidden="true">打开 GitHub →</span><h3>${escapeHTML(skill.name)}</h3><p>${escapeHTML(skill.subtitle)}</p><div class="card-tags" aria-label="技能标签">${tags}</div><div class="card-facts"><span><small>分类</small>${escapeHTML(skill.category)}</span><span><small>模型</small>${escapeHTML(modelLabel(skill))}</span><span><small>版本</small>${escapeHTML(skill.version || '未声明')}</span></div></a><div class="card-bottom"><span>${escapeHTML(skill.sourceLabel)}</span><span class="card-actions"><a href="${encodeURI(skill.url)}">查看详情 <span aria-hidden="true">↗</span></a><button data-action="copy" data-skill="${escapeHTML(skill.id)}"${promptDisabled}>复制 Prompt</button><a class="card-github-link" href="${escapeHTML(skill.sourceUrl)}" target="_blank" rel="noopener noreferrer" title="点击后进入技能库，按照 README 使用说明开始操作。" aria-label="打开 ${escapeHTML(skill.sourceLabel)} GitHub 技能库">打开 GitHub 技能库 <span aria-hidden="true">↗</span></a></span></div></article>`;
    };
    ...
    grid.innerHTML = [...groups.entries()].filter(([, skills]) => skills.length).map(([key, skills], index) => `
      <section class="skill-category-group category-${escapeHTML(key)}" style="--category-delay:${index * 70}ms" aria-labelledby="skill-category-${escapeHTML(key)}">
