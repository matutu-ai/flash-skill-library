(() => {
  ...
  function renderCatalog() {
    ...
    const renderCard = skill => {
      ...
      return `<article class="skill-card"><a class="card-open" href="${escapeHTML(skill.sourceUrl)}" target="_blank" rel="noopener noreferrer" aria-label="打开 ${escapeHTML(skill.sourceLabel)} GitHub 技能库"><div class="card-top"><span class="skill-icon ${escapeHTML(skill.categoryKey)}">${icon(skill.icon)}</span><span class="card-index">REPO / ${String(catalog.indexOf(skill) + 1).padStart(3, '0')}</span></div><h3>${escapeHTML(skill.name)}</h3><p>${escapeHTML(skill.subtitle)}</p><div class="card-tags" aria-label="技能标签">${tags}</div><div class="card-facts"><span><small>分类</small>${escapeHTML(skill.category)}</span><span><small>模型</small>${escapeHTML(modelLabel(skill))}</span><span><small>版本</small>${escapeHTML(skill.version || '未声明')}</span></div></a><div class="card-bottom"><span>${escapeHTML(skill.sourceLabel)}</span></div></article>`;
    };
